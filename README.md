...
A browser-based estimating tool that converts project scope inputs into crew sizes and activity durations for the bid system line-item entry. Built for pavement maintenance estimating with field-calibrated production rates, three-tier scheduling, and deterministic job analysis.

---

## What It Does

The calculator sits upstream of the bid system. You enter quantities, phases, and travel time. It returns crew sizes and durations across three production tiers (conservative, standard, aggressive). Cost summaries exist as sanity checks against 347-job historical benchmarks — the bid system owns all final cost buildup, overhead, and bid pricing.
...
- **Full Export** — Comprehensive plain-text report structured for the bid system workflow: crew/duration first, then analysis, then costs (labeled as reference)
...
**Costs are secondary.** the bid system owns cost buildup. Calculator costs exist to sanity-check crew sizing and duration, not to produce bid pricing. Exports label them accordingly.
